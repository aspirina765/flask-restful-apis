...
from src.model.data1 import Data1, Data1Schema
from src.model.data2 import Data2, Data2Schema
...
datas = [
    Data1('prediction', 5000),
    Data1('prediction', 200),
    Data2('prediction', 50),
    Data2('prediction', 100)
]
...
@APP.route('/data1')
def get_data1():
    schema = Data1Schema(many=True)
    datas1 = schema.dump(
        filter(lambda t: t.type == DataType.DATA1, datas)
    )
    return jsonify(datas1.data)

@APP.route('/data2')
def get_data2():
    schema = Data2Schema(many=True)
    datas2 = schema.dump(
        filter(lambda t: t.type == DataType.DATA2, datas)
    )
    return jsonify(datas2.data)
